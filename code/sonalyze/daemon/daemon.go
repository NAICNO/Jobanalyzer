...
// -no-add
//
//   EXPERIMENTAL.  This disables the /add endpoint, -upload-auth and -match-user-and-cluster.  The
//   implication is that we're either running on a read-only database or we're using -kafka to
//   handle all ingestion.
//
// Termination:
//
