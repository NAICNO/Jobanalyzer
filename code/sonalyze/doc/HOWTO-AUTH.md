...
The following pertains to sonalyze running as a daemon.  In the following, `$JOBANALYZER` is the
value of the `-jobanalyzer-dir` parameter to sonalyze.

What's described here is the existing setup but a new OAUTH-based system is more or less working.

## Query authentication

### Design

Jobanalyzer user identities U are currently global as opposed to cluster-local, this was a necessity
for the web dashboard that provides views to several clusters' data on the same domain.  It will
change.

Each user has one password P associated with it. Passwords are stored in the server's secrets - any
kind of identity management can in principle be used.
...
There is none: every known user has superuser privileges on all clusters, ie, they can see all data
...
## Kafka authentication and autorization

Kafka must be configured with the correct passwords for the appropriate topics.

Note that if the cluster name is embedded in the data there is not generally a check that it matches
the name given in the topic.

## Better authorization

**NOTE:** This has not been implemented, it is aspirational (but will likely be replaced by an
OAUTH-style system anyway).
