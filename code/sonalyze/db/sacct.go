...
type SacctInfo struct {
	Timestamp    int64  // Unix time - timestamp of record
...
LineLoop:
	...
	FieldLoop:
		for {
			...
			case 'A':
				if val, ok := match(tokenizer, start, lim, eqloc, "Account"); ok {
					...
					matched = true
				} else if val, ok := match(tokenizer, start, lim, eqloc, "AveCPU"); ok {
			...
			case 't':
				if val, ok := match(tokenizer, start, lim, eqloc, "time"); ok {
					info.Timestamp, err = parseRFC3339(val)
					matched = true
				}
			case 'U':
		...
		irritants := ""
		if info.Version == UstrEmpty || info.Timestamp == math.MaxInt64 || info.JobID == 0 {
			if info.Version == UstrEmpty {
				irritants += "version "
			}
			if info.Timestamp == math.MaxInt64 {
				irritants += "time "
			}
			if info.JobID == 0 {
				irritants += "jobid "
