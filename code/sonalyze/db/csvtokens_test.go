...
func checkMatch(t *testing.T, tokenizer *CsvTokenizer, s string, eqloc int) {
	...
	s2 := tokenizer.BufSubstringSlow(a, b)
	if s2 != s {
...
Loop:
	for {
		...
		default:
			...
			if tokenizer.BufSubstringSlow(a, b) != "abcdefghijklmnopqrstuvwxyz" {
				t.Fatal("Bad string")
			}
			if c != CsvEqSentinel {
...
func checkMatchTag(t *testing.T, tokenizer *CsvTokenizer, tag, s string) {
	...
	s2 := tokenizer.BufSubstringSlow(c, b)
	if s2 != s {
		t.Fatalf("Not a match for value '%s': '%s'", s, s2)
	}
}
...
// Test that `=` at the beginning is not recognized as a name/value separator, and that it is
// recognized as a name if followed by another =.

func TestCsvTokenizer10(t *testing.T) {
	text := `=37
"=37"
==37`
	tokenizer := NewTokenizer(bytes.NewReader([]byte(text)))
	checkMatch(t, tokenizer, "=37", CsvEqSentinel)
	checkSentinel(t, tokenizer, CsvEol)
	checkMatch(t, tokenizer, "=37", CsvEqSentinel)
	checkSentinel(t, tokenizer, CsvEol)
	checkMatch(t, tokenizer, "==37", 2)
	checkSentinel(t, tokenizer, CsvEof)
}
