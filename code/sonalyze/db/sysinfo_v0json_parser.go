...
//
// FIXME: This is a really lossy translation.  First, the old SysinfoEnvelope can't hold all the
// data that come from the new Sysinfo data.  Then, the NodeConfigRecord can hold even less.  What
// we need to do here is upgrade the pipeline in Jobanalyzer so that we no longer use NodeConfigRecord,
// ideally we move to the new SysinfoEnvelope but minimally to the old one.  Of course, then we
