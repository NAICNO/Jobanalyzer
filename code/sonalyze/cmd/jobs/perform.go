...
func (jc *JobsCommand) Perform(
	...
	// SampleCount in the SampleJob record.  Be careful when applying the filter to synthesized
...
// data.  We don't cache anything now b/c the underlying config code has a hashmap already.  In the
// future, for jobs with a slurm aspect, we'll instead return the allocated resources, when
// available, falling back on config values when necessary.  In the future after that, we're not
// going to have a config in the static sense, but a sysinfo blob that was valid at the time of the
// sample.  That can be lazily computed and for slurm jobs it may not need to be computed at all.
