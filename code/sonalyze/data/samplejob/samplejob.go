// A materialized table of jobs constructed from one or more sample streams.  Streams are merged (or
// not) and then aggregated into a coherent job view.
//
...
// the sample stream is not needed by the client, as it is usually not.
package samplejob
...
type SampleJob struct {
	...
	// Fields aggregated across all time steps from the raw data (so 1 core / 1 card = 100%)
	CpuPctSum    float64 // Sum of CPU utilization
	CpuKBSum     uint64  // Sum of main memory in use
	RssAnonKBSum uint64  // Sum of resident main memory
	GpuPctSum    float64 // Sum of GPU utilization
	GpuKBSum     uint64  // Sum of GPU memory use

	// Maximum values across all time steps
	CpuPctMax    float64 // CPU utilization
	CpuKBMax     uint64  // Main memory in use
	RssAnonKBMax uint64  // Resident memory use
	GpuPctMax    float64 // GPU utilization
	GpuKBMax     uint64  // GPU memory use
}

type NeededComputations struct {
	// Compute aggregate command name in the `Cmd` field
	Cmd bool
	// Compute aggregate host name in the `Hosts` field
	Hosts bool
	// Raw samples in the `Job` field, if false that field is nil
	Job bool
	// Compute the zombie flag in `ComputedFlags`
	Zombie bool
}
...
func Query(
	...
	summaries := aggregateAndFilterJobs(isMergeable, streams, bounds, needed, merge, verbose)
	if verbose {
...
func aggregateAndFilterJobs(
	...
	for _, job := range jobs {
		...
		duration := last - first
		aggregate := computeAggregate(host, *job, needed.Cmd, needed.Hosts, needed.Zombie)
