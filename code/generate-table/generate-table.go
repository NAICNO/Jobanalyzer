	...
	for _, field := range fields.Fields {
		...
		for _, attr := range field.Attrs {
			if !validAttr[attr.Name] {
				log.Fatal("Field %s: Invalid attribute name %s", field.Name, attr.Name)
			}
		...
		if config, found := attrs["config"]; found {
			...
			case "false":
				// nothing
			default:
				log.Fatal("Field %s: Bad attribute value for 'config'", field.Name)
			}
		...
		if ptrName := attrs["indirect"]; ptrName != "" {
			...
			fmt.Fprintf(output, "\t\t\t\treturn %s(%s(d.%s.%s), ctx)\n",
				formatName(field.Type), field.Type, ptrName, actualFieldName)
			fmt.Fprintf(output, "\t\t\t}\n")
			fmt.Fprintf(output, "\t\t\treturn \"?\"\n")
		} else {
			fmt.Fprintf(output, "\t\t\treturn %s(%s(d.%s), ctx)\n",
				formatName(field.Type), field.Type, actualFieldName)
		}
		fmt.Fprintf(output, "\t\t},\n")
		if d := attrs["desc"]; d != "" {
...
func helpSection(tableName string, help *parser.HelpSect) {
	first := 0
	last := len(help.Text) - 1
	for first < len(help.Text) && isBlank(help.Text[first]) {
		first++
	}
	for last > first && isBlank(help.Text[last]) {
		last--
	}
