...
// - arrow will not play nice with column headers containing breakable spaces,
//   I guess we could put it below?
// - coloring for cells and rows that are special: again perhaps `major` could be used (these rows
//   are colored)?
...
//  - "display": optional, a function that takes the row and field tag and returns a value that is
//    its displayable representation.  This must be either an HTML element (which is appended to the
//    table cell) or a value (which is converted to string and becomes the textcontent of a table
//    cell)
	...
	for (let field of this.fields) {
	    ...
	    let content = String(row[field.tag])
            if (field.hasOwnProperty("display")) {
                content = field.display(row, field.tag)
            }
