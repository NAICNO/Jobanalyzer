...
import { useClusterJobs, useClusterGpuTimeseries } from '../hooks/useClusterQueries'
...
): ColDef[] => [
  {
    ...
    onCellClicked: (params) => {
      if (params.data?.job_id) {
        window.open(`${APP_BASE_PREFIX}${cluster}/jobs/${params.data.job_id}`, '_blank')
      }
    },
  },
  {
...
const createTopUsersColDefs = (cluster: string): ColDef[] => [
  {
    ...
    onCellClicked: (params) => {
      if (params.data?.user) {
        window.open(`${APP_BASE_PREFIX}${cluster}/jobs/query?user=${encodeURIComponent(params.data.user)}&searched=1`, '_blank')
      }
    },
  },
  {
