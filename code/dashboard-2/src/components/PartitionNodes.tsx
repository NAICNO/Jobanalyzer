        ...
        <Accordion.ItemContent>
          <Accordion.ItemBody>
            <VStack align="start" gap={3} w="100%">
                  ...
                  <HStack gap={2} flexWrap="wrap">
                    {nodes.map((node: string) => (
                      <Tag.Root key={node} size="sm" asChild>
                        <RouterLink to={`/${partition.cluster}/nodes/${node}`}>
              ...
              {nodes.length > 0 && (
                <Box mt={2}>
                  <RouterLink to={`/${partition.cluster}/nodes`} style={{ color: 'var(--chakra-colors-blue-500)', textDecoration: 'underline', fontSize: '0.875rem' }}>
                    View all nodes →
                  </RouterLink>
                </Box>
              )}
