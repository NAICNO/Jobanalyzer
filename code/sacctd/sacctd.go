...
//    redundancies.  The default window is configurable below.
//
// Notes for users:
//
...
//
// The following triplet (reformatted) is instructive (missing JobIDRaw):
//
...
func main() {
	...
	cmd := exec.Command(
		...
		"-S",
		fmt.Sprintf("now-%dminutes", *window),
		"-E",
		"now",
	)
	...
	timeField := "time=" + time.Now().Format(time.RFC3339)
	...
	for scan.Scan() {
		...
		csvfields := make([]string, 0, len(fields)+2)
		csvfields = append(csvfields, versionField)
		csvfields = append(csvfields, timeField)
