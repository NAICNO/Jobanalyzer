// joblog is an abstraction that reads and cleans up job log files with possibly-overlapping and
// partly-redundant data records, as generated by the cpuhog and deadweight reports.
//
// The output of ReadJoblogFiles is a list of hosts, and for each host a list of individual jobs for
// that host (the former sorted ascending by name, the latter descending by GetLastSeen() value).
...
//
// We can assume that if the log records are ingested and partitioned by host and then bucketed by
// timestamp and the buckets sorted, then if a job ID is present in two consecutive buckets in the
...
// analysis producing the logs runs often enough that in two consecutive runs, a job ID will not be
// found to be reused in the second run.
...
// timestamp of the record at parsing time.
...
func ReadJoblogFiles[T Job](
	...
	for _, filePath := range files {
		...
		for _, unparsed := range records {
			if parsed, ok := parseLogRecord(unparsed); ok {
				if len(bucket) > 0 {
	...
	// Sort host list ascending by name
...
func processLogRecordsForHost[T Job](
	...
	// Sort the buckets descending by time.
	...
	for bucketIdx, bucket := range buckets {
		for _, record := range bucket {
			...
			// Scan the bucket list forward starting at the one after the current bucket, collecting
			// records matching 'record' until none are found in a bucket.
		probeLoop:
	...
	// Sort the resulting jobs descending by time.  TODO: This may actually be redundant but
	// it doesn't hurt.
	sort.Slice(results, func(i, j int) bool {
		return results[i].GetLastSeen().After(results[j].GetLastSeen())
	})

	// Set the Expired mark on subsequent jobs for any given job ID.
